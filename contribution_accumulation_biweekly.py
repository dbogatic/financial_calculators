...
def forecast_contributions(dob, years_of_service, eligible_pay, rate_of_return, pay_growth_rate, target_age, starting_balance, hire_date=None):
    ...
    reference_age = today.year - dob.year - ((today.month, today.day) < (dob.month, dob.day))  # Correctly determine if birthday has passed
    ...
    # Number of bi-weekly periods in a year
    periods_per_year = 26
    bi_weekly_rate_of_return = (1 + rate_of_return) ** (1 / periods_per_year) - 1
    ...
    for year in range(current_year, current_year + years_until_target + 1):
        ...
        # Iterate through each bi-weekly period in the year
        for period in range(1, periods_per_year + 1):
            # No contributions until 2026, only salary inflation before that
            if year >= 2026:
                annual_regular_contribution = (salary * regular_contribution_rate) / periods_per_year
                annual_service_credits = (salary * service_credit_rate) / periods_per_year
                total_contribution = annual_regular_contribution + annual_service_credits
                total_contributions += total_contribution

            # Apply bi-weekly rate of return on accumulated assets
            total_contributions *= (1 + bi_weekly_rate_of_return)

            # Update salary in October if period corresponds to October or beyond
            if year == current_year and today.month < 10 and period == 21:  # Assuming ~October in bi-weekly terms
                salary *= (1 + pay_growth_rate)
            elif year > current_year and period == 21:
                salary *= (1 + pay_growth_rate)
        ...
        data.append({
            ...
            'Salary': salary,
            'Starting Balance': starting_balance if year == current_year else 0,  # Display starting balance only in the first year
    ...
    df['Starting Balance'] = df['Starting Balance'].map('${:,.2f}'.format)
